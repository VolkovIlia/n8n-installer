...
if is_profile_active "vpn"; then
  ...
  if [ -n "$WG_EASY_HOSTNAME" ]; then
    echo "   HTTPS (Caddy): https://${WG_EASY_HOSTNAME}"
    echo "   User: ${WG_EASY_USERNAME:-admin}"
    echo "   Password: ${WG_EASY_PASSWORD:-<auto-generated>}"
    echo "   Direct HTTP: http://${WG_HOST:-<not_set_in_env>}:51821 (Password: ${WG_PASSWORD:-<auto-generated>})"
  else
    echo "   Direct HTTP: http://${WG_HOST:-<not_set_in_env>}:51821"
    echo "   Password: ${WG_PASSWORD:-<auto-generated>}"
    echo "   (HTTPS via Caddy not configured - set WG_EASY_HOSTNAME in .env to enable)"
  fi
  echo
  echo "🤖 Telegram Bot:"
